...
#include "Abilities/BeamAbility.h"
#include "Abilities/AoEAbility.h"
#include "Abilities/ConeAbility.h"
...
void AUE5TopDownARPGPlayerController::SetupInputComponent()
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(InputComponent))
	{
		...
		EnhancedInputComponent->BindAction(ActivateAbilityHealAction, ETriggerEvent::Started, this, &AUE5TopDownARPGPlayerController::OnActivateAbilityHealStarted);
		EnhancedInputComponent->BindAction(ChangeAbility, ETriggerEvent::Started, this, &AUE5TopDownARPGPlayerController::OnChangeAbility);
...
void AUE5TopDownARPGPlayerController::OnActivateAbilityHealStarted()
{
	UE_LOG(LogUE5TopDownARPG, Log, TEXT("OnActivateAbilityHealStarted"));
	...
	if (IsValid(ARPGCharacter))
	{
		...
		if (bHitSuccessful)
		{
			ARPGCharacter->ActivateAbilityHeal(Hit.Location);
		}
	}
}

void AUE5TopDownARPGPlayerController::OnChangeAbility()
{
	UE_LOG(LogUE5TopDownARPG, Log, TEXT("Ability Switched From %f To %f"), abilityNumber, abilityNumber + 1);
	AUE5TopDownARPGCharacter* ARPGCharacter = Cast<AUE5TopDownARPGCharacter>(GetPawn());
	abilityNumber++;
	if (abilityNumber > 3)
		abilityNumber = 0;
	ARPGCharacter->SetAbilityInstance(abilityNumber);
